...
const void *kTRDelegateProxyKey = "com.tr.framework.delegate-container";
static NSString *const kTRDelegateProxyToken = @"com.tr.framework.delegate-container.token";

NSString *const kTRDelegateContainerNonConformantObjectException                    = @"TRDelegateContainerNonConformantObjectException";
NSString *const kTRDelegateContainerObjectDoesNotImplementRequiredSelectorException = @"TRDelegateContainerObjectDoesNotImplementRequiredSelectorException";
NSString *const kTRDelegateContainerConfigurationException                          = @"TRDelegateContainerConfigurationException";

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
@interface TRDelegateProxy ()
...
- (id)initWithObject:(id)object
{
    if((self = [super init])) {
        ...
        //        TRLogTrace(TRLogContextDefault, @"<wrapper: object(%p) class = %@, hash = %ld>", object, NSStringFromClass(_wrappedObjectClass), _wrappedObjectHash);
    }
    return self;
}
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
- (TRDelegateContainer *)delegateContainer
{
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
- (TRDelegateProxy *)delegateProxy
{
    TRDelegateProxy *proxy = nil;
    @synchronized(kTRDelegateProxyToken) {
        proxy = objc_getAssociatedObject(self, kTRDelegateProxyKey);
        if(!proxy) {
            proxy = [[TRDelegateProxy alloc] init];
            objc_setAssociatedObject(self, kTRDelegateProxyKey, proxy, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
    }
    return proxy;
}
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
@implementation TRDelegateProxy
{
...
- (void)configureWithProtocol:(Protocol *)protocol
{
    if(self.delegatesCount > 0) {
        [[NSException exceptionWithName:kTRDelegateContainerConfigurationException
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector
{
    ...
    TRLogTrace(TRLogContextDefault, @"<[delegate_container (%p)]: unrecognized selector (%@). Did you forget to configure the delegation protocol ?>",
               self,
...
#ifdef TR_DELEGATE_CONTAINER_SYNC_CALLBACKS
    [self notifyDelegatesWithInvocationSync:anInvocation
...
//             self, delegate, NSStringFromProtocol(self.protocol));
    ...
    if(canRegisterDelegate == NO) {
        ...
        NSException *exception = [NSException exceptionWithName:kTRDelegateContainerNonConformantObjectException
                                                         reason:reason
                                                       userInfo:nil];
        [exception raise];
    }
...
////////////////////////////////////////////////////////////////////////////////
// Used when TR_DELEGATE_CONTAINER_SYNC_CALLBACKS is defined, this runs everything synchronously
// which should help with debugging (pdcgomes 19.12.2013)
////////////////////////////////////////////////////////////////////////////////
- (void)notifyDelegatesWithInvocationSync:(NSInvocation *)invocation callbackQueue:(dispatch_queue_t)callbackQueue completionHandler:(dispatch_block_t)completionHandler
{
    NSString *selectorName = NSStringFromSelector(invocation.selector);
...
- (void)_performCleanup
{
    ...
    [delegates enumerateObjectsUsingBlock:^(TRWeakObjectWrapper *wrapper, BOOL *stop) {
        if(!wrapper.object) {
            TRLogTrace(TRLogContextDefault, @"<delegate_container: object of class %@ (hash:%lu) has been deallocated>",
            ...
            if(self.protocol != nil &&
               [self.requiredMethods containsObject:selectorName]) {
                NSString *reason = [NSString stringWithFormat:@"Attempted to register a delegate (%@) that does not implement a required selector (%@) from the declared protocol (%@)",
                                    ...
                                    selectorName,
                                    NSStringFromProtocol(self.protocol)];
                NSException *exception = [NSException exceptionWithName:kTRDelegateContainerObjectDoesNotImplementRequiredSelectorException
                                                                 reason:reason
                                                               userInfo:nil];
                [exception raise];
            }
